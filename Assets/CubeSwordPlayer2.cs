...
public class CubeSwordPlayer2 : MonoBehaviour
{
    ...
    public GameObject coinObjectParent;
    public int value;
    ...
    void Start()
    {
        ...
        //tempObject = GameObject.Find("CubeSwordPlayer2");
        sliderHealth = GameObject.Find("CubeSwordPlayer2HealthBar" + count.ToString()).GetComponent<Slider>(); //set initial slider value
        healthCanvas = GameObject.Find("CubeSwordPlayer2HealthCanvas" + count.ToString());
    ...
    void Update()
    {
        transform.position += (Vector3.forward * (-1)) * Time.deltaTime * moveSpeed; //start moving

        hitEnemy = Physics.OverlapSphere(attackPos.position, attackRange, isEnemy);//hitbox

        if (health <= 0) 
        {
            if (coinObjectParent == null) { coinObjectParent = GameObject.Find("Player1Canvas"); }
            coinObjectParent.transform.GetChild(5).GetComponent<Currency>().coins = coinObjectParent.transform.GetChild(5).GetComponent<Currency>().coins + value;
            Destroy(gameObject);           
        }
